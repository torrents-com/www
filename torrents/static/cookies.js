;(function($){$.cookiesLaw={cid:false,notified:false,hide_msg:false,start_scripts:false,
    gen_id:function(){var c=navigator.appName+navigator.version+navigator.platform+navigator.userAgent+(navigator.cookie?navigator.cookie:"")+(document.referrer?document.referrer:"");d=c.length;for(var e=window.history.length;0<e;)c+=e--^d++;return (Math.round(2147483647*Math.random())^hash(c)&2147483647)+"."+(Math.round((new Date)["getTime"]()/1E3));},
    run_scripts:function(){if(!this.start_scripts)return;this.start_scripts();this.start_scripts=false;},
    initialize:function(ds,ss){this.cid=$.cookie("_cid")||this.gen_id();this.start_scripts=ss;this.domains=ds;var cookie_level=$.cookie("cookie_level");this.notified=cookie_level>0;this.hide_msg=cookie_level>1;if (this.notified){this.final_process();return;}this.visit_domains("notify", function(){$.cookie("cookie_level", $.cookiesLaw.hide_msg?2:1,{expires:3650,path:'/'});$.cookiesLaw.final_process();});},
