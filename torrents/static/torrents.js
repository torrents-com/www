...
function data_flag(){var elm=$(this);var data=elm.data("flag");if (data.length==0) return; elm.click(function(e){e.preventDefault();e.stopImmediatePropagation();$.colorbox({html:"<div id='flag_confirm' class=''><div><h1>Flag confirmation</h1></div><p>This content has been flagged as <strong>"+data+"</strong>. Do you want to download it anyway?</p>"+elm[0].outerHTML+"<a>No, thanks</a></div>", width:"600px",close:false,overlayClose:true,fixed:true});$("#flag_confirm a[data-track]").each(data_track);$("#flag_confirm a").click(function(e){$.colorbox.close()});return false})}
