...
function data_flag(){var elm=$(this);var data=elm.data("flag");if (data.length==0) return; elm.click(function(e){e.preventDefault();e.stopImmediatePropagation();var gbody = $("body");$.colorbox({html:"<div id='flag_confirm' class=''><div><h1>"+gbody.data("flagged_title")+"</h1></div><p>"+gbody.data("flagged_desc").replace("$FLAG$", "<strong>"+data+"</strong>")+"</p>"+elm[0].outerHTML+"<a>"+gbody.data("no_thanks")+"</a></div>", width:"600px",close:false,overlayClose:true,fixed:true});$("#flag_confirm a[data-track]").each(data_track);$("#flag_confirm a").click(function(e){$.colorbox.close()});return false})}
...
$(function(){
    var gbody = $("body");
    init_dialogs(gbody);
    window.PAGE_MESSAGES = {"sent": [gbody.data("message_sent"), "info"],"write":[gbody.data("write_something"),"error"]};
    window.suggestmeyes_loaded=true;

    if ($("#files >.filepaths li").length>1){$("#files >.filepaths >li").addClass("open");$("#files >.filepaths").treeview({"collapsed":true});}
    if (adult_content&&!$.cookie("adult_confirm")){trackGAEvent('Adult confirm', "Ask");$.colorbox({html:"<div id='adult_confirm' class='adult_confirm title big_icon porn'><span></span><div><h1>"+ gbody.data("adult_confirm_title") +"</h1></div><p>"+gbody.data("adult_confirm_desc")+"</p><a id='yes_button' href='#'>"+gbody.data("adult_confirm_yes")+"</a><a data-track='Adult confirm,No,' href='/'>"+gbody.data("adult_confirm_no")+"</a></div>", width:"600px",close:false,overlayClose:false,fixed:true,escKey:false});$("#adult_confirm a[data-track]").each(data_track);$("#yes_button").click(function(e){e.preventDefault();$.cookie("adult_confirm",1,{path:'/'});trackGAEvent('Adult confirm',"Yes");$.colorbox.close()});}
